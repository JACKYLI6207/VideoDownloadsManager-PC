 ...
 * 不做去重、不丟棄，方便逐步追蹤指定資料夾流程。
 */
let _vdmDiagChain = Promise.resolve();
VDM.diag = (message, detail = "") => {
  const ctx =
    typeof window !== "undefined" && window.location
      ? (window.location.pathname.includes("offscreen") ? "offscreen" : "panel")
      : "bg";
  try {
    console.log(`[VDM:${ctx}]`, message, detail || "");
  } catch {
    /* ignore */
  }
  ...
  _vdmDiagChain = _vdmDiagChain.then(async () => {
    try {
      const { vdmLogs = [] } = await chrome.storage.local.get("vdmLogs");
      vdmLogs.unshift(entry);
      if (vdmLogs.length > 250) vdmLogs.length = 250;
      await chrome.storage.local.set({ vdmLogs });
    } catch {
      /* ignore */
    }
  });
  return _vdmDiagChain;
};
...
VDM.clampConcurrentTasks = (n) => {
  const v = Math.floor(Number(n));
  if (!Number.isFinite(v) || v < 1) return 2;
  return v;
};

/** 全局 HLS/HTTP 片段並行上限（任務數 × 單任務連線，無硬上限） */
...
/** 更新任務即時作業描述（顯示於「進行中」卡片） */
VDM.setTaskActivity = (task, text) => {
  if (!task) return;
  task.activity = String(text || "");
  if (typeof VDM._activityFn === "function") VDM._activityFn(task);
};
