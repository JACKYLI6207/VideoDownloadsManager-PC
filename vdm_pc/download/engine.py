...
class DownloadEngine(QObject):
    ...
    def _restore_persisted(self) -> None:
        ...
        self._emit_stats()
    def _reconcile_task_cache(self, task: DownloadTask) -> None:
        """重開後以磁碟暫存為準還原進度，清掉舊的網路/檔案錯誤訊息。"""
        if not (task.video.is_m3u8 or ".m3u8" in (task.video.url or "").lower()):
            task.error = ""
            return
        cache = cache_root(self.settings)
        if not disk_store.has_cache_data(cache, task.video.id):
            task.error = "程式已重新開啟，可點「繼續」或「從頭下載」"
            return
        merged = disk_store.read_merge_meta(cache, task.video.id)
        folder = disk_store.task_dir(cache, task.video.id)
        total = task.total or 0
        if total > 0:
            task.merged = merged
            task.downloaded = merged + disk_store.count_on_disk_segments(folder, merged, total)
            task.download_progress = self._seg_pct(task.downloaded, total)
            task.merge_progress = self._merge_seg_pct(merged, total)
        task.error = "程式已重新開啟，請點「繼續」接續下載（暫存已保留）"
    ...
    def _run_task(self, task_id: str) -> None:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    if task_id not in self._cancelled:
                        self._pause_on_error(task, str(exc), auto_retry=True)
            ...
            self._pump()
    @staticmethod
    def _is_local_file_error(exc: BaseException) -> bool:
        if isinstance(exc, FileNotFoundError):
            return True
        if isinstance(exc, OSError) and getattr(exc, "errno", None) == 2:
            return True
        text = str(exc)
        return "Errno 2" in text or "No such file or directory" in text

    def _pause_on_error(self, task: DownloadTask, msg: str, *, auto_retry: bool = True) -> None:
        ...
        if auto_retry:
            ...
            task.error = f"{msg}（請點「繼續」接續下載，暫存已保留）"
        self._persist_now()
    ...
    def _download_hls(self, task: DownloadTask, gen: int) -> None:
        ...
        if not disk_store.has_cache_data(cache, video.id):
            if task.merged == 0 and task.downloaded == 0:
        ...
        downloaded_bytes = merger.merged_bytes
        speed_state = {"t": time.time(), "b": downloaded_bytes}
        ...
        merger.drain_ready(on_merge)
        task.merged = merger.next_append
        task.downloaded = merger.buffered_count
        need = disk_store.segments_to_fetch(cache, video.id, merger.next_append, len(segments))
        if not need:
            if merger.next_append >= len(segments):
                merger.finish(on_merge)
                self._finalize_hls_mp4(task, cache, video.id, out, gen)
            return
        pool_workers = min(workers, max(1, len(need)))
        folder = disk_store.task_dir(cache, video.id)

        def work(client: httpx.Client, idx: int) -> int:
            if not self._alive(task.id, gen):
                raise RuntimeError("cancelled")
            seg_path = folder / disk_store.seg_name(idx)
            if seg_path.is_file():
