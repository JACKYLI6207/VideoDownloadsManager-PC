...
class DownloadEngine(QObject):
    ...
    def _pause_on_error(self, task: DownloadTask, msg: str) -> None:
        self._paused.add(task.id)
        task.status = "paused"
        task.error = f"{msg}（已暫停，請點「繼續」接續下載）"
        self._schedule_persist()
    ...
    def _download_hls(self, task: DownloadTask, gen: int) -> None:
        ...
        playlist = m3u8_lib.parse_m3u8(self._fetch_text(video.url, headers), video.url)
        if playlist.is_variant:
            variant_url = m3u8_lib.pick_best_variant(playlist)
            if not variant_url:
                raise RuntimeError("M3U8 沒有可用變體")
            playlist = m3u8_lib.parse_m3u8(self._fetch_text(variant_url, headers), variant_url)
        ...
        task.downloaded = disk_store.buffered_count(cache, video.id, len(segments), merged_through)
        ...
        workers = min(self._worker_limit(), max(1, len(need)))
        ...
        def work(idx: int) -> int:
            ...
            try:
                data = self._fetch_bytes(segments[idx].url, headers)
                if not self._alive(task.id, gen):
                    raise RuntimeError("cancelled")
                disk_store.write_segment(cache, video.id, idx, data)
                return len(data)
            finally:
                self._release_slot()

        with ThreadPoolExecutor(max_workers=workers) as pool:
            futures = {pool.submit(work, i): i for i in need}
            for fut in as_completed(futures):
                if not self._alive(task.id, gen):
                    raise RuntimeError("cancelled")
                size = fut.result()
                downloaded_bytes += size
                task.downloaded = disk_store.buffered_count(cache, video.id, len(segments), task.merged)
                task.download_progress = self._seg_pct(task.downloaded, len(segments))
    ...
    def _download_http(self, task: DownloadTask, gen: int) -> None:
        ...
        with httpx.Client(timeout=120, follow_redirects=True) as client:
            ...
            if total > 1_048_576 and accept_ranges:
                self._download_ranges(task, video.url, headers, total, out, gen)
                return
            ...
            task.total = len(data) or task.downloaded
            task.download_progress = 100
            task.merge_progress = 100
            task.progress = 99

    def _download_ranges(self, task: DownloadTask, url: str, headers: dict, total: int, out: Path, gen: int) -> None:
        ...
        def work(start: int, end: int, index: int) -> None:
            if not self._alive(task.id, gen):
                raise RuntimeError("cancelled")
            self._acquire_slot(task, gen)
            try:
                ...
                with httpx.Client(timeout=180, follow_redirects=True) as client:
                    ...
                    self._check_response(res)
                    buffers[index] = res.content
            finally:
                self._release_slot()

        with ThreadPoolExecutor(max_workers=connections) as pool:
            futs = [pool.submit(work, s, e, idx) for s, e, idx in parts]
            for fut in as_completed(futs):
                if not self._alive(task.id, gen):
                    raise RuntimeError("cancelled")
                fut.result()
                downloaded = sum(len(buffers[k]) for k in sorted(buffers))
